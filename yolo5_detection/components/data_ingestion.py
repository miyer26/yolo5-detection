import os 
import sys
import zipfile
import gdown
from yolo5_detection.logger import logging
from yolo5_detection.exception import AppException
from yolo5_detection.entity.config_entity import DataIngestionConfig
from yolo5_detection.entity.artifacts_entity import DataIngestionArtifact


class DataIngestion:
    def __init__(self, data_ingestion_config: DataIngestionConfig = DataIngestionConfig()):
        try:
            self.data_ingestion_config = data_ingestion_config
        except Exception as e:
            raise AppException(e, sys)

    def download_data(self) -> str:
        """Featch data from URL"""

        try:
            dataset_url = self.data_ingestion_config.data_download_url
            zip_download_dir = self.data_ingestion_config.data_ingestion_dir
            os.makedirs(zip_download_dir, exist_ok = True)
            data_file_name = "data.zip"
            zip_file_path = os.path.join(
                zip_download_dir, data_file_name
            )
            logging.info(f"Downloading data from {dataset_url} into file {zip_file_path}")


            gdrive_file_id = dataset_url.split("/")[-2]
            prefix = "https://drive.google.com/uc?/export=download&id="
            gdown.download(prefix + gdrive_file_id, zip_file_path)
            logging.info(f"Download data from {dataset_url} to file {zip_file_path}")
            return zip_file_path
        
        except Exception as e:
            raise AppException(e, sys)

    def extract_zip_file(self, zip_file_path: str) -> str:
        """
        Function to zip file

        Args:
            zip_file_path (str): path to zip file
        """

        try:
            data_store_path = self.data_ingestion_config.data_store_file_path
            os.makedirs(data_store_path, exist_ok=True)
            with zipfile.ZipFile(zip_file_path, 'r') as zip_ref:
                zip_ref.extractall(data_store_path)
            logging.info(f"Extracting zip file: {zip_file_path} into dir: {data_store_path}")

            return data_store_path
        except Exception as e:
            raise AppException(e, sys)

    def initate_data_ingestion(self) -> DataIngestionArtifact:
        logging.info("Initiating data_ingestion method")
        try:
            zip_file_path = self.download_data()
            data_store_path = self.extract_zip_file(zip_file_path)

            data_ingestion_artifact = DataIngestionArtifact(
                data_zip_file_path = zip_file_path,
                data_store_path = data_store_path
            )

            logging.info("Completed data ingestion")
            logging.info(f"Data ingestion artifact: {data_ingestion_artifact}")

            return data_ingestion_artifact

        except Exception as e:
            raise AppException(e, sys)


    def run_pipeline(self) -> None:
        try:
            data_ingestion_artifact = self.initate_data_ingestion()

        except Exception as e:
            raise AppException(e, sys)